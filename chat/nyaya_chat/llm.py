...
# surface is its only source of truth for the law. Citations are emitted
# inline as ``[[act: <short_name>, ref: <ref>]]`` markers, parsed by the
# frontend into citation chips.
SYSTEM_PROMPT = (
...
def get_model(settings: Settings | None = None):
    """Return a cached ``ChatNVIDIA`` instance with thinking mode applied.

    If ``Settings.llm_enable_thinking`` is ``True``, the model is wrapped with
    ``.with_thinking_mode(enabled=True)`` so it emits ``reasoning_content`` via
    ``additional_kwargs`` during streaming. The ``settings`` arg is read on the
    first call only. Tests should call ``reset_model_cache()`` to start fresh.
    """
